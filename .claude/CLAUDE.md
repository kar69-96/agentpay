...
```bash
pnpm --filter agentpay vitest run src/vault/vault.test.ts
```

**Build/test a single package:**
```bash
pnpm --filter agentpay build        # SDK only
pnpm --filter @agentpay/web build   # Web only
```
...
This is a pnpm workspace monorepo with turbo orchestration. Four packages: `packages/sdk` (published as `agentpay`), `packages/web` (Next.js), `examples/basic`, `examples/mcp`.
